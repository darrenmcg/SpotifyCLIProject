...
def main():
    url = validate_url(input("Please Enter A Spotify Song URL: ").strip())
    if "track" in url:
...
def get_track_info(track_url):
    ...
    if res.status_code != 200:
        raise ValueError("Invalid Spotify Song URL")
...
def download_yt(yt_link):
    ...
    youTube.title = "".join([c for c in youTube.title if c not in ['/', '\\', '|', '?', '*', ':', '>', '<', '"']])
    # don't download existing files if the user wants to skip them
    exists = os.path.exists(f"../music/{youTube.title}.mp3")
    ...
    mp4_no_frame.close()
    os.remove(vid_file)
    os.replace(audio_file, f"../music/tmp/{youTube.title}.mp3")
    audio_file = f"../music/tmp/{youTube.title}.mp3"
    return audio_file
